    ...
    k.AmSlicedChart = k.Class({
        ...
        initChart: function () {
            ...
            this.dataChanged &&
                ...
                (this.dataChanged = !1),
                this.setLegendData(this.chartData));
            this.drawChart();
        },
        ...
        startSlices: function () {
            ...
            for (a = 0; a < this.chartData.length; a++)
                0 < this.startDuration && this.sequencedAnimation
                    ? this.setStartTO(a)
        ...
        clickSlice: function (a, b, c) {
            this.checkTouchDuration(b) &&
                (isNaN(a) || (a = this.chartData[a]),
                a.pulled ? this.pullSlice(a, 0) : this.pullSlice(a, 1),
                k.getURL(a.url, this.urlTarget),
                c ||
                    this.fire({
                        type: 'clickSlice',
                        dataItem: a,
        ...
        removeSmallSlices: function () {
            ...
            for (b = a.length - 1; 0 <= b; b--)
                a[b].percents < this.groupPercent &&
                    ((this.groupValue += a[b].value), a.splice(b, 1));
        },
            ...
            if (k.ifArray(a)) {
                if (0 < this.realWidth && 0 < this.realHeight) {
                    ...
                    m ||
                        ...
                        (g = g - z - n),
                        (m = Math.min(d, g)),
                        g < d && ((m /= 1 - this.angle / 90), m > d && (m = d)),
                        (g = k.toCoordinate(this.pullOutRadius, m)),
                        (m = (0 <= w ? m - 1.8 * (w + g) : m - 1.8 * g) / 2));
                    ...
                    for (var E, h = 0; h < a.length; h++)
                        if (
                            ...
                            if (
                                this.labelsEnabled &&
                                this.labelText &&
                                e.percents >= this.hideLabelsPercent
                            ) {
                                var l = n + r / 2;
                                0 > l && (l += 360);
                                360 < l && (l -= 360);
        ...
        checkOverlapping: function (a, b, c, d, g) {
            ...
            if (n) {
                if (!0 === d)
                    for (h = a + 1; h < k; h++)
                        e[h].labelQuarter == c &&
                            (f = this.checkOverlappingReal(b, e[h], c)) &&
                            (h = k);
                else
                    for (h = a - 1; 0 <= h; h--)
                        e[h].labelQuarter == c &&
                ...
                !0 === f &&
                    ...
                    ((f = b.ty + 3 * b.iy),
                    (b.ty = f),
                    n.translate(b.textX, f),
                    this.checkOverlapping(a, b, c, d, g + 1));
            }
        },
